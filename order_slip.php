...
<section class="order-slip-section mt-5 mb-5">
    <h3 class="text-center mb-5">Thank you for shopping with us!</h3>
...
<?php 
  ...
  if(isset($_SESSION['cart']) && !empty($_SESSION['cart'])){
    ...
    foreach($_SESSION['cart'] as $pid => $item){
      ...
      echo '<th class="table-header-prod-title" scope="row"><img class="view-cart-img me-3" src="images/' . $item["image"] . '">' . $item['name'] . '</th>';

      echo '<td class="table-header-cart-text">R '.$item['price'].'</td>';
      echo '<td class="table-header-cart-text text-center">'.$item['quantity'].'</td>';
      echo '<td class="table-header-cart-text table-header-cart-text-price"> R '.$formatted_item_subtotal.'</td>';
      echo '</tr>';
      echo '</form>';
    }
    $item_names_string = implode(", ", $item_names);
    $temp_addy = "sci-bono";
    $insert_query = "INSERT INTO `sales_tb`(`user_fullname`, `products_ordered`, `price`, `delivery_address`, `order_number`) VALUES ('$name','$item_names_string','$grand_subtotal','$temp_addy','$order_number')" ;
    $insert_into_sales_tb = mysqli_query($conn, $insert_query);
