...
<body>
  ...
  <div class="carousel-inner hero-slider w-100">
    ...
    <div class="carousel-item">
      <img src="images\hero-image-ezgif.com-webp-to-jpg-converter.jpg" class="d-block w-100 slider-img" alt="...">
    </div>
    <div class="carousel-item">
      <img src="images\LV bg skyscraper.jpg" class="d-block w-100 slider-img" alt="...">
    </div>
  </div>
  ...
  $collect_from_db = mysqli_query($conn, "SELECT * FROM `products_tb` WHERE product_brand = 'LV' ") or die ('Query Failed');
  ...
  if (mysqli_num_rows($collect_from_db) > 0) {
    while($row = mysqli_fetch_assoc($collect_from_db)){
      ...
      <div class='card-body'>
        <h6 class='card-title text-ellipsis'>{$row['product_name']}</
        h6>
...
$collect_from_db = mysqli_query($conn, "SELECT * FROM `products_tb` WHERE product_brand = 'LV' AND product_discount = 'true' ") or die('Query Failed');
